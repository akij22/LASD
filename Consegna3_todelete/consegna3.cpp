...
int DFS(int n) {
    ...
    if (details)
        ...
        if (V_visitato[n]==1) // se gia' visitato (nodo grigio) --> ciclo!
        return 1;

        if (V_visitato[n]==2) // se gia' visitato (nodo nero) --> altro percorso (DAG)
        return 0;
    ...
    V_visitato[n] = 2; // abbandono il nodo per sempre (ovvero lo rendo nero)
    graph_print();
...
// `index` = indice per socrrere l'array `seq` e confrontare ogni volta il valore dell'array con il valore del nodo attuale

void DFS_consegna3(int n, int* seq, int n_elem_seq, int index, bool& found) {
    ...
    cout << "Valore attuale da ricercare: " << seq[index] << endl;
    // Se il valore del nodo corrente è uguale al corrispettivo valore nell'array, aggiorno i contatori

    cout << "Valore attuale di `index`: " << index << endl;
    if (V[n] != seq[index]) {
        cout << "Nodo " << V[n] << " scartato..." << endl;
        index = 0;
        return;

    } else {
        cout << "Nodo corretto trovato!" << endl;
        index++;
    }
    ...
    while (elem != NULL) { /// elenco tutti i nodi nella lista
        ...
        cout << "Nodo da esplorare: " << elem->val << endl;
        DFS_consegna3(elem->val, seq, n_elem_seq, index, found);
...
int main(int argc, char **argv) {
    ...
    if (graph) {
        ...
        /// preparo header per la stampa del grafo con Dot
    ...
    // Creazione di un grafo manualmente
    list_insert_front(E[0], 2);
    list_insert_front(E[0], 1);
    list_insert_front(E[2], 5);
    ...
    // Print di debug riguardo la costruzione del grafo
    // for (int i = 0; i < n_nodi; i++) {
    ...
    DFS_consegna3(0, seq, n_elem_seq, 0, found);

    cout << ( (found) ? "Trovata sequenza nel grafo" : "Non e' stata trovata sequenza nel grafo" ) << endl;
