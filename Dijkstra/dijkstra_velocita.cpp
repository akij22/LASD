...
// compilazione: g++ dijkstra_velocita.cpp
//
// Obiettivo:
...
void node_print(int n) {
    ...
    output_graph << "fillcolor = \""<< 0.1+0.7*col1 << " " << 1.0 << " " << 0.7+0.3*col1 << "\"; style=filled; ";
    if (V_dist[n] < INFTY)
        output_graph << "label = "
                     << "\"" << n << "\" ];\n";
    else
...
void shortest_path(int n) {
    ...
    */
    ...
    while (q_size != 0) {
        ...
        cout << "best_idx: " << best_idx << " best_dist: " << best_dist << endl;
        if (best_idx >= 0) {
            /// estrai dalla coda
            int u = best_idx;
            V_visitato[u] = 1;
            ...
            while (elem != NULL) {
                ...
                float alt = V_dist[u] + elem->w / ((V[u]+V[v])/2);
                // float alt = V_dist[u] + elem->w * pow(V[u]-V[v],2);
...
int main(int argc, char **argv) {
    ...
    // shortest_path(N*(N/2)+N/2);
    shortest_path(N*(N-1)+N-1);
