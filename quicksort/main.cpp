...
// compilazione: g++ lezione4-6-quicksort.cpp
//
// Obiettivo:
...
// Algoritmo di sorting quicksort
...
// Funzione per il parsing degli argomenti passati in input mediante ./a.outx
int parse_cmd(int argc, char **argv) {
    
    // Controllo argomenti passati in input mediante ./a.out
    if (argc < 2) {
        ...
        cout << "            n (ovvero dimensione specifica dell'array calcolata per ogni test) = k * max-dim / d, k=1 .. d\n";
        cout << "  -t=<int>: Specifica quanti volte viene lanciato l'algoritmo per ogni specifica dimensione n [default: 1]\n";
    ...
    for (int i = 2; i < argc; i++) {
        ...
        // Definizione dell'ordine dell'array in input all'algoritmo
        if (argv[i][1] == 'r') {
            rndm = 1;
            ascending = 0;
        }
...
/*
* argc = numero di argomenti passati dall'utente
...
int main(int argc, char **argv) {
    ...
    // DEBUG
    cout << "Random: " << rndm << " Ascending: " << ascending << " Descending: " << descending << "\n";
    ...
    for (n = max_dim / ndiv; n <= max_dim; n += max_dim / ndiv) {
        ...
        for (test = 0; test < ntests; test++) {
            ...
            for (i = 0; i < n; i++) {
                ...
                if (ascending)
                    // Test con array ordinato crescente
                    A[i] = i;
                
                else if (random)
                    // Test con array random
                    A[i] = rand() % (n);
                
                else if (descending)
                    // Test con array ordinato decrescente
                    A[i] = n - i;
